            ...
            try
            {
                ...
                using (WebClient web = new WebClient() { Encoding = Encoding.UTF8 })
                {
                    ...
                    foreach (var game in games)
                    {
                        ...
                        var homeTeamLogo = gameLink.Children.Where(c => c.ClassName == "scores-container").First().Children.Where(v => v.ClassName == "home").
                            First().Children.Where(b => b.ClassName == "team-score").
                            First().Children.Where(n => n.ClassName == "logo-container").
                            First().Children.Where(t => t.ClassName == "logo").First();
                        var team1Name = await ResolveTeamName(homeTeamLogo.Attributes["src"].Value);

                        var awayTeamLogo = gameLink.Children.Where(c => c.ClassName == "scores-container").First().Children.Where(v => v.ClassName == "away").
                            First().Children.Where(b => b.ClassName == "team-score").
                            First().Children.Where(n => n.ClassName == "logo-container").
                            First().Children.Where(t => t.ClassName == "logo").First();
                        var team2Name = await ResolveTeamName(awayTeamLogo.Attributes["src"].Value);
                        gamesList.Add(new LiigaGame() { StartTime = splitString[3], HomeTeamName = team1Name, AwayTeamName = team2Name });
                    }
                }
                return gamesList;
            }
            catch (Exception ex) { Printer.PrintTag("Exception", ex.Message); return null; }
        }

        private static async Task<string> ResolveTeamName(string imageSource)
        {
            ...
            switch (searchResult)
            {
                case "Karpat": searchResult = "Kärpät"; break;
                case "Assat": searchResult = "Ässät"; break;
            }
            return searchResult;
        }
    }
